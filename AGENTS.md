...
PinTerm 是一个 IntelliJ Platform 插件（group 为 `io.github.yaml`，id 为 `io.github.yaml.pinterm`）。它把配置好的终端标签页打开到编辑器区域并固定，在会话进入运行态后可选地发送一行 shell 命令。它依赖 IDE 自带的 *Reworked Terminal*（`org.jetbrains.plugins.terminal`，since-build 为 `262`）。打开并固定终端用的是 Reworked Terminal 的实验性 API（`@ApiStatus.Experimental`）。Marketplace 会拒绝 `@ApiStatus.Internal`，因此不要再调用 `TerminalTabsStorage`、`StartupManager.runAfterOpened`、`AppLifecycleListener.appStarted`、`UISettings.getState()` / `UISettingsState`，也不要调用 `TerminalToolWindowTabBuilder.shouldAddToToolWindow`。
...
`PinTermPlatformTerminals` 是 **唯一接触 Reworked Terminal 实验性 API 的类**：`TerminalToolWindowTabsManager`、`TerminalToolWindowTab`、`TerminalView`、`TerminalViewVirtualFile`。终端 API 的使用应只集中在这里。
...
- **`PinTermCommandDispatch.decide(state)`** 把 `TerminalViewSessionState` 映射到 `SEND` / `FAIL_TERMINATED` / `WAIT`，方式是按状态类名的后缀匹配（`$Running`、`$Terminated`）。这是故意基于字符串匹配的，因为这些状态类是实验性 API。
...
- 在项目关闭 / 插件卸载时（`PinTermDynamicPluginSupport.beforePluginUnload`），`closeAllOpenTerminals()` 只关闭归属本插件的编辑器文件。它不再改写 Terminal 工具窗口的持久化会话；`TerminalTabsStorage` 是 Internal API，Marketplace 会因此拒绝更新。
- **启动清理：** `PinTermStartupActivity`（`postStartupActivity`，实现公开的 `ProjectActivity`）在项目打开后调用 `scheduleStartupCleanup`。5 秒窗口内的 0/500/1500/3000/5000 毫秒处重试，只关闭仍带 `OWNED` 标记的编辑器文件。重启后该标记丢失，所以不会按名称关掉 Terminal 工具窗口里恢复出来的标签。
...
- `PinTermDefaults` 持有 `DEFAULT_TABS`（Codex/Claude/Grok，当没有 tab 时作为种子），以及 `displayTabName` 截断辅助方法。插件不再改 IDE 全局的 **Show pinned tabs in a separate row**（`UISettingsState` 是 Internal API）。
...
## 约定 / 注意点
- 代码风格：工具类都是包级私有（`final class X` 配私有构造器）；只有 `TerminalTabState`、`PinTermSettings`、`PinTermConfigurable`、`PinTermToolbarDropdownAction`、`PinTermService`、`PinTermStartupActivity` 是 public 的。
...
- 提升 `sinceBuild`（当前为 `262`）可能需要重新接触 `PinTermPlatformTerminals` 里的 Reworked Terminal 实验性 API，以及 `PinTermService` 清理路径中的已废弃 `Disposer.isDisposed` 调用。2026.3 的 `TerminalToolWindowTabBuilder.restoreOnProjectReopen(false)` 是公开方法，能让标签不进入 Terminal 的重启恢复；262 上还没有这个方法。不要为了这个去调用 `shouldAddToToolWindow(false)`，那个方法是 Internal。
